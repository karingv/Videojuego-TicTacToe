...
int             iHeight = screenHeight;     // alto de la ventana
...
void dibujar_marco(int WIDTH, int HEIGHT, int margen, int espacio_superior) {
    ...
    color(BLANCO);
    // LÌneas del marco
    rectangulo_lleno(x_ini, y_ini -5 , x_fin, y_ini + 2); // LÌnea superior
    rectangulo_lleno(x_ini, y_fin , x_fin, y_fin); // LÌnea inferior
    rectangulo_lleno(x_ini, y_ini, x_ini + 8, y_fin); // LÌnea izquierda
    rectangulo_lleno(x_fin -8, y_ini, x_fin, y_fin); // LÌnea derecha
}
...
void dibujar_enemigo_alien(float x, float y, float longitud_cuerpo, float altura_cuerpo, float longitud_antena, float altura_antena) {
    ...
    float antena_izq_1_izq = x - longitud_antena / 2 - longitud_cuerpo / 2;
    float antena_izq_1_der = x + longitud_antena / 2 - longitud_cuerpo / 2;
    ...
    float antena_der_izq = x - longitud_antena / 2 + longitud_cuerpo / 2;
    float antena_der_der = x + longitud_antena / 2 + longitud_cuerpo / 2;
...
static COLORREF _colores[] = {
   ...
   RGB(18, 35, 157 ), // AZUL OSCURO
   RGB(163, 169, 172 ), // GRIS_OSCURO
};
